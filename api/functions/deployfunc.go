...
func (h *Handler) DeployFuncHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(metadataFile).Decode(&buildReq); err != nil {
	...
	// Untar funcFile to /tmp/{buildReq.Name}
	targetDir := fmt.Sprintf("/tmp/%s", buildReq.Name)
	if err := os.MkdirAll(targetDir, 0755); err != nil {
		http.Error(w, fmt.Sprintf("failed to create target dir: %v", err), http.StatusInternalServerError)
		return
	}

	// If the tarball is gzipped, wrap with gzip.Reader
	...
	w.Write([]byte(fmt.Sprintf("Function %s uploaded successfully", buildReq.Name)))
}
